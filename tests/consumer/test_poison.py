"""S2 acceptance: a poison pill is dropped and XACKed after its 6th delivery.

Recovery path is unit-tested with a fake redis client (no live services, no new
dependency). Delivery count > 5 (from XPENDING ... IDLE 60000 detail) is the
drop trigger; entries at the boundary (==5) must survive.
"""
...
class _FakeRedis:
    """Stub of the redis calls used by Consumer.recover()."""
...
def test_poison_pill_dropped_after_sixth_delivery():
    ...
    consumer = Consumer(fake_redis, conn=None, consumer_name="pg_writer-test")

    dropped_before = _sample("mlobs_consumer_events_dropped_total")

    consumer.recover()  # drop_poison() + reclaim()
    ...
    assert _sample("mlobs_consumer_events_dropped_total") - dropped_before == 1.0
